...
const app = ref<any>({});
const appDetail = ref<any>({ enable: true });
const version = ref('');
...
const appKey = ref();
const operate = ref();
const architectures = ref<string[]>([]);
...
const getDetail = async (id: number, currentVersion: string) => {
    loadingDetail.value = true;
    try {
        const res = await getAppDetail(id, currentVersion, 'app');
...
const openInstall = () => {
    if (!jumpToInstall(app.value.type, app.value.key)) {
        installRef.value.acceptParams({ app: app.value });
        open.value = false;
    }
};
