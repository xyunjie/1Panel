    ...
    <div class="d-name">
        <div class="d-name-row flex items-center justify-between">
            <div class="name-actions flex items-center justify-start gap-1">
                ...
                <span class="ml-1">
                    <el-tooltip v-if="mode !== 'upgrade'" effect="dark" :content="$t('commons.button.log')" placement="top">
                        <el-button
                            type="primary"
                            link
                            @click="$emit('openLog')"
            ...
            <div class="operate-actions flex flex-wrap items-center justify-end gap-1">
                <el-button class="h-button" plain round size="small" @click="$emit('openUploads')" v-if="mode === 'installed'">
                    {{ $t('database.loadBackup') }}
                </el-button>
                <el-button class="h-button" plain round size="small" @click="$emit('openBackups')" v-if="mode === 'installed'">
                ...
                <el-button
                    ...
                    round
                    size="small"
                    :disabled="(installed.status !== 'Running' && installed.status !== 'UpgradeErr') || installed.appStatus === 'TakeDown'"
